...
@Composable
internal fun <T> EditWheelPicker(
    items: List<T>,
    selectedIndex: Int,
    ...
    Box(
        ...
        Box(
            modifier = Modifier.fillMaxWidth().height(itemHeight).background(InsColors.Gray100, AppShape.Card),
        )
        LazyColumn(
            state = listState,
            flingBehavior = rememberSnapFlingBehavior(listState, snapPosition = SnapPosition.Center),
        ) {
            items(spacerCount) { Box(modifier = Modifier.height(itemHeight).fillMaxWidth()) }
...
@Preview(name = "수정 화면 시간 선택 휠", showBackground = true)
@Composable
private fun EditWheelPickerPreview() {
