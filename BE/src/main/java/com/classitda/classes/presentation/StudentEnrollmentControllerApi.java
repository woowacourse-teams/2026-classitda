...
public interface StudentEnrollmentControllerApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - 로그인한 학생 본인의 특정 수업 신청 생명주기를 classSessionId와 enrollmentId로 조회합니다.
                    ...
                    - 숨긴 신청, 다른 수업·회원의 신청, 존재하지 않는 신청은 모두 같은 404를 반환합니다.

                    ### local Swagger 테스트 데이터

                    - 김회원(ID 1)의 ACCESS 토큰을 Swagger Authorize에 입력합니다.
                    - studioId는 1을 사용합니다.
                    - classSessionId와 enrollmentId별 기대 결과는 다음과 같습니다.

                    | classSessionId | enrollmentId | 기대 결과 |
                    |---:|---:|---|
                    | 103 | 4 | 예약 완료(RESERVED) |
                    | 104 | 16 | 대기 중(WAITING) |
                    | 117 | 19 | 승인 필요(OFFERED) |
                    | 108 | 6 | 출석 완료(ATTENDED) |
                    | 121 | 25 | 결석(ABSENT) |
                    | 118 | 22 | 예약 취소(RESERVATION_CANCELED) |
                    | 106 | 29 | 수업 취소(SESSION_CANCELED) |
                    | 119 | 23 | 대기 취소 이력이므로 404 |
                    | 120 | 24 | 제안 만료 이력이므로 404 |
                    | 101 | 1 | 다른 회원의 신청이므로 404 |
                    """
    )
    @ApiResponses({
            @ApiResponse(
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "시설 또는 조회 가능한 본인 신청을 찾을 수 없습니다. 숨긴 신청과 다른 수업·회원의 신청도 동일하게 처리합니다.",
                    content = @Content(
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = {
    ...
    StudentEnrollmentDetailResponse findOne(
            ...
            Long studioId,
            @Parameter(description = "신청이 속한 수업 회차 ID입니다.", required = true, example = "117")
            Long classSessionId,
