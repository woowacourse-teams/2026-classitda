...
@RequestMapping("/api/studios/{studioId}/class-session-enrollments")
@RestController
public class StudentEnrollmentController implements StudentEnrollmentControllerApi {
    ...
    @Override
    @GetMapping(path = "/{enrollmentId}", version = "1")
    public StudentEnrollmentDetailResponse findOne(
            @CurrentMemberId Long memberId,
            @PathVariable Long studioId,
            @PathVariable Long enrollmentId
    ) {
        return StudentEnrollmentDetailResponse.from(
                queryService.findOne(memberId, studioId, enrollmentId)
        );
    }
}
