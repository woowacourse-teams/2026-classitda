...
Docker 명령어를 몰라도 저장소에 포함된 스크립트로 클래스잇다 백엔드 서버와 MySQL을 실행하고 중지할 수 있습니다.
...
MySQL이 정상 상태가 된 후 서버가 자동으로 시작됩니다. 로컬 프로필에서는 서버를 시작할 때 기존 테이블을 삭제하고 `schema.sql`로 다시 생성합니다.
...
서버와 MySQL 컨테이너를 삭제하지 않고 중지합니다. 전용 네트워크와 MySQL Docker 볼륨도 유지됩니다.

## 포트 변경

기본 포트를 사용할 수 없다면 실행 명령 앞에 다른 포트를 지정합니다.

```bash
LOCAL_APP_PORT=18080 LOCAL_DB_PORT=13306 ./docker/scripts/start.sh
```
