...
이력 판정은 `class_session_enrollment`, `member_pass_product`, `class_session` 세 곳을 확인하고, 놓친 참조는 `DataIntegrityViolationException`을 잡아 소프트 삭제로 넘겨요.
...
- (-) `membership_id`를 참조하는 테이블이 늘면 판정에 검사를 추가해야 해요. 빠뜨리면 예외 폴백이 받지만 의도한 경로는 아니에요.
- (-) `notice`를 이번 판정에서 뺐어요. 공지 기능이 생기면 검사를 추가해야 해요.
