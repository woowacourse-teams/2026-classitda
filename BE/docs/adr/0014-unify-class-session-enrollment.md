...
신청 상태, 상태 변경 시각과 제안 만료 시각은 embedded `EnrollmentState` 값 객체로 묶어요. 값 객체가 유효한 상태 전이를 검증하고 변경된 새 값을 반환하며, Aggregate는 수강권 연결과 출결처럼 다른 구성 요소를 함께 조정하는 책임을 가져요.
...
- (=) 대기 등록에서는 현재 어떤 수강권으로든 예약할 수 있는지 확인하되 특정 수강권을 선택하거나 연결하지 않아요. 이후 수강권의 남은 횟수가 없어져도 `WAITING`과 `OFFERED` 상태에는 영향을 주지 않아요.
- (=) `OFFERED`는 좌석을 점유하므로 조회의 `reservedCount`에 `RESERVED`와 함께 포함해요. 다만 수강권 횟수는 확보하거나 차감하지 않으며, 회원이 예약 화면으로 이동할 때 현재 어떤 수강권으로든 예약할 수 있는지를 다시 계산해 응답해요.
- (=) 제안을 수락해 `RESERVED`로 전환할 때 회원이 사용할 수강권을 직접 선택해요. Application Service가 같은 트랜잭션에서 그 수강권을 다시 검증하고 횟수를 차감한 뒤 신청에 연결해요.
- (=) `WAITING`, `OFFERED`, `EXPIRED`는 수강권 FK가 없고 `RESERVED`는 반드시 있어요. `CANCELED`는 대기 취소와 예약 취소를 모두 표현하므로 수강권 FK가 없거나 있을 수 있어요.
...
- 신청 상태와 출결 전이는 도메인 테스트로 검증해요.
- 예약 화면 진입 가능 여부는 조회 시점의 값일 뿐 예약 성공을 보장하지 않아요. 예약 확정 트랜잭션에서 선택한 수강권을 반드시 다시 검증해요.
